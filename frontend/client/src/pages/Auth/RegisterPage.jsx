...
function RegisterPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch("http://13.235.24.233:5000/api/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: form.name, email: form.email,
          password: form.password,   
          phone: form.phone, projectType: form.projectType
        })
      });
  ...
  const s = {
    ...
    // strength bar
    barWrap:     { height:"5px", background:"#e2e8f0", borderRadius:"99px",
                   marginBottom:"8px", overflow:"hidden" },
    barFill:     { height:"100%", borderRadius:"99px", transition:"width .4s, background .4s",
                   width:`${(passedCount/5)*100}%`, background: strengthColor },
    barLabel:    { fontSize:"12px", fontWeight:"600", color: strengthColor,
                   textAlign:"right", marginBottom:"8px" },

    // rules box
    rulesBox:    { background:"#f8fafc", border:"1px solid #e2e8f0", borderRadius:"8px",
                   padding:"10px 14px", marginBottom:"12px" },
    rulesTitle:  { fontSize:"11px", fontWeight:"700", color:"#64748b", marginBottom:"6px",
                   textTransform:"uppercase", letterSpacing:"0.06em" },
    rulesGrid:   { display:"grid", gridTemplateColumns:"1fr 1fr", gap:"5px" },
    ruleRow:     (ok) => ({ display:"flex", alignItems:"center", gap:"6px",
                            fontSize:"12px", color: ok ? "#10b981" : "#94a3b8",
                            fontWeight: ok ? "600" : "400" }),
    ruleDot:     (ok) => ({ width:"7px", height:"7px", borderRadius:"50%",
    ...
    <div style={s.container}>
      ...
      <div style={s.formBox}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <select style={s.input} name="projectType" onChange={handleChange} required>
          ...
          <div style={s.pwdWrap}>
            <input
              style={s.pwdInput} name="password"
          ...
          <div style={s.pwdWrap}>
            <input
              style={s.pwdInput} name="confirmPassword"
